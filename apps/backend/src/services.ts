...
export class IngestService {
  ...
  ingest(request: UploadBatchRequest, nowMs: number): UploadBatchResponse {
    ...
    // Batch identifiers are generated by phones and are only unique within a
    // gateway session. Scope the idempotency key to that gateway so two phones
    // cannot accidentally suppress one another's observations.
    const scopedBatchId = `${request.gatewayToken}:${request.batchId}`;
    const replay = this.store.isDuplicateBatch(scopedBatchId);
    this.store.rememberBatch(scopedBatchId);
    ...
    for (const item of request.items) {
      ...
      // The HTTP field is an index/idempotency hint; packet identity is owned
      // by the canonical bytes. Never store valid bytes under a caller-chosen
      // different ID, otherwise deduplication, acknowledgements and map audit
      // trails disagree between the phone and website.
      if (validation.packet.header.packetId !== item.packetId) {
        results.push({ packetId: item.packetId, outcome: 'invalid', reason: 'packet id does not match canonical bytes' });
        continue;
      }
