    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#050811' }}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={{ minHeight: 64, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, borderBottomWidth: 1, borderBottomColor: '#142039' }}>
          ...
          <View style={{ flex: 1, marginLeft: 6 }}><Text style={{ color: '#F8FAFC', fontSize: 17, fontWeight: '900' }}>Mesh peer {shortToken(peerToken)}</Text><Text style={{ color: peer ? '#00F2FE' : '#FFB300', fontSize: 11, marginTop: 2 }}>{peer ? 'Recently reachable by Bluetooth' : 'Offline · will relay when a route appears'}</Text></View>
        </View>
