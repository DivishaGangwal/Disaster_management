...
function CampaignComposer({ campaigns, selected, incidents, records, onCreate, onUpdate, onSelect }: { campaigns: Campaign[]; selected?: Campaign; incidents: Incident[]; records: RegionalRecord[]; onCreate: (input: CampaignDraftInput) => void; onUpdate: (id: string, input: CampaignDraftInput) => void; onSelect: (id: string) => void }) {
  ...
  const input = (): CampaignDraftInput => ({ title, summary, severity, category, instruction, profile: 'audible-normal', dataType, ...(dataType === 'regional-record' ? { objectId } : {}), ...(carriesPoint && point ? { latE7: floatToE7(point.lat), lonE7: floatToE7(point.lon), radiusM } : dataType === 'official-alert' ? { clearLocation: true } : {}) });
  const submit = (e: FormEvent) => { e.preventDefault(); if (overTextBudget) return; editingId ? onUpdate(editingId, input()) : onCreate(input()); };
  const beginEdit = () => { if (!selected) return; setEditingId(selected.campaignId); setTitle(selected.title); setSummary(selected.summary); setSeverity(selected.severity); setCategory(selected.category); setInstruction(selected.instruction); setDataType(selected.dataType ?? 'official-alert'); setObjectId(selected.objectId ?? records[0]?.objectId ?? ''); setPoint(typeof selected.latE7 === 'number' && typeof selected.lonE7 === 'number' ? { lat: e7ToFloat(selected.latE7), lon: e7ToFloat(selected.lonE7) } : null); setRadiusM(selected.radiusM ?? 5000); };
