...
import { TIME, type CampaignState } from '@dsm/contracts';
...
import type { Campaign, GatewayAudit, Incident, OperatorSession, Overview, PacketStreamItem, RegionalRecord, Responder, SectionKey } from './types';
...
export function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setOverview(result[0]); setIncidents(result[1]); setResponders(result[2]); setRecords(result[3]); setCampaigns(result[4]); setPackets(result[5]); setGatewayAudit(result[6]); setConnection('connected');
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <nav aria-label="Primary operations">{NAV.map((item) => <button key={item.key} className={section === item.key ? 'nav-item active' : 'nav-item'} onClick={() => setSection(item.key)}><b aria-hidden="true" /><span><strong>{item.label}</strong><small>{item.note}</small></span></button>)}</nav>
      <div className={`sidebar-status ${connection}`}><span className="live-dot" /><div><strong>{connection === 'connected' ? 'Live' : connection === 'disconnected' ? 'Offline' : 'Connecting'}</strong><small>{connection === 'connected' ? 'Last update just now' : 'Retrying automatically'}</small></div></div>
    </aside>
    <main id="workspace">
      <header className="topbar"><div><span className="jurisdiction">Assam state command</span><h1>{NAV.find((item) => item.key === section)?.label}</h1></div><div className="top-actions"><LiveClock /><span className="operator-label">{session.operatorLabel}</span><span className="deployment">IN-AS</span><button className="button ghost" onClick={() => void refresh()}>Refresh</button><button className="button ghost" onClick={() => { api.signOut(); setSession(undefined); }}>Sign out</button></div></header>
...
function CoordinateV2({ incidents, responders, records, packets, gatewayAudit, onAssign, onResponderAction }: { incidents: Incident[]; responders: Responder[]; records: RegionalRecord[]; packets: PacketStreamItem[]; gatewayAudit: GatewayAudit; onAssign: (responder: string, incident: string) => void; onResponderAction: (responder: string, action: 'accepted' | 'en-route' | 'arrived' | 'resolved') => void }) {
  ...
  return <section className="command-atlas" aria-label="Assam live operations">
    ...
    <div className="atlas-grid">
      ...
      <aside className="operations-rail">
        ...
        <div className="rail-section gateways-live">
          ...
          {gatewayAudit.gateways.map((gateway) => { const activity = gatewayActivity(gatewayAudit, gateway.gatewayToken); return <button className={selectedId === gateway.gatewayToken ? 'gateway-pulse selected' : 'gateway-pulse'} key={gateway.gatewayToken} onClick={() => setSelectedId(gateway.gatewayToken)}><i className={activity.ageMs < GATEWAY_ACTIVE_WINDOW_MS ? 'online' : 'stale'} /><span><strong>{gatewayLabel(gateway.nodeToken)}</strong><small>{activity.uploads} up · {activity.downloads} down</small></span><time>{formatAge(Math.floor(activity.ageMs / 1000))}</time></button>; })}
        </div>
      </aside>
    </div>
...
function CampaignWorkspace({ campaigns, incidents, records, onRefresh, perform }: { campaigns: Campaign[]; incidents: Incident[]; records: RegionalRecord[]; onRefresh: () => Promise<void>; perform: (label: string, action: () => Promise<unknown>) => Promise<void> }) {
  ...
  const approvedCampaigns = campaigns.filter((campaign) => ['approved', 'broadcaster-ready', 'audio-generated', 'decode-tested', 'scheduled', 'played', 'archived'].includes(campaign.state));
  const [broadcastId, setBroadcastId] = useState(approvedCampaigns[0]?.campaignId ?? '');
  const broadcastCampaign = approvedCampaigns.find((campaign) => campaign.campaignId === broadcastId) ?? approvedCampaigns[0];
...
function CampaignWorkspaceV2({ campaigns, incidents, records, onRefresh, perform }: { campaigns: Campaign[]; incidents: Incident[]; records: RegionalRecord[]; onRefresh: () => Promise<void>; perform: (label: string, action: () => Promise<unknown>) => Promise<void> }) {
  const [selectedId, setSelectedId] = useState(campaigns[0]?.campaignId ?? '');
  const [view, setView] = useState<'register' | 'editor' | 'decoder'>('register');
  const selected = campaigns.find((campaign) => campaign.campaignId === selectedId) ?? campaigns[0];
  const approved = campaigns.filter((campaign) => ['approved', 'broadcaster-ready', 'audio-generated', 'decode-tested', 'scheduled', 'played', 'archived'].includes(campaign.state));
  const [broadcastId, setBroadcastId] = useState(approved[0]?.campaignId ?? '');
  const broadcastCampaign = approved.find((campaign) => campaign.campaignId === broadcastId) ?? approved[0];
  ...
  return <section className="campaign-desk">
    <header className="campaign-desk-header">
      <div><h2>Campaign register</h2><p>{campaigns.length} versions · {liveCount} moving through approval and broadcast</p></div>
      <nav aria-label="Campaign workspaces">
        <button className={view === 'register' ? 'active' : ''} onClick={() => setView('register')}>Register</button>
        <button className={view === 'editor' ? 'active' : ''} onClick={() => setView('editor')}>Compose</button>
        <button className={view === 'decoder' ? 'active' : ''} onClick={() => setView('decoder')}>Audio decoder</button>
      </nav>
    </header>

    {view === 'register' && <div className="campaign-register-layout">
      <section className="campaign-register-v2" aria-label="Campaign versions">
        <header><span>All campaigns</span><button className="button primary" onClick={() => setView('editor')}>New campaign</button></header>
      ...
      <section className="campaign-focus">
        {selected ? <>
          <header className="campaign-focus-header"><div><State value={selected.state} /><h3>{selected.title}</h3><p>{selected.summary}</p></div><button className="button secondary" onClick={() => setView('editor')}>Revise content</button></header>
          <CampaignFlow campaign={selected} />
          <CampaignApproval campaign={selected} onAction={(state) => perform(`Move campaign to ${state}`, () => api.transitionCampaign(selected.campaignId, state))} onPrepare={() => perform('Acoustic program preparation', () => api.prepareBroadcast(selected.campaignId))} />
        </> : <Empty text="Create the first campaign to begin the register." />}
      </section>
    </div>}

    {view === 'editor' && <div className="campaign-editor-layout">
      ...
      <div className="campaign-editor"><CampaignComposer campaigns={campaigns} selected={selected} incidents={incidents} records={records} onSelect={setSelectedId} onCreate={(input) => perform('Campaign draft', () => api.createCampaign(input))} onUpdate={(id, input) => perform('Campaign revision', () => api.updateCampaign(id, input))} /></div>
    </div>}

    {view === 'decoder' && <section className="decoder-workspace">
      <header><div><button className="back-link" onClick={() => setView('register')}>← Back to register</button><h3>Audio decoder</h3><p>Listen through the microphone or inspect a WAV file against the selected approved packet.</p></div><label><span>Approved campaign</span><select value={broadcastCampaign?.campaignId ?? ''} onChange={(event) => setBroadcastId(event.target.value)}><option value="" disabled>Select campaign</option>{approved.map((campaign) => <option value={campaign.campaignId} key={campaign.campaignId}>{campaign.title} · {flowName(campaign.state)}</option>)}</select></label></header>
      <BroadcastConsole campaign={broadcastCampaign} onRefresh={onRefresh} receiverOnly />
    </section>}
  </section>;
}
function CampaignFlow({ campaign }: { campaign?: Campaign }) { const steps: CampaignState[] = ['draft', 'validated', 'approved', 'broadcaster-ready', 'audio-generated', 'decode-tested', 'scheduled', 'played', 'archived']; const current = campaign ? Math.max(0, steps.indexOf(campaign.state)) : -1; return <div className="campaign-flow">{steps.map((step, i) => <div key={step} className={i < current ? 'done' : i === current ? 'current' : ''}><i>{i < current ? '✓' : i + 1}</i><span>{flowName(step)}</span></div>)}</div>; }

const REGION_CENTRE = { lat: 26.1, lon: 92.5 };

function CampaignComposer({ campaigns, selected, incidents, records, onCreate, onUpdate, onSelect }: { campaigns: Campaign[]; selected?: Campaign; incidents: Incident[]; records: RegionalRecord[]; onCreate: (input: CampaignDraftInput) => void; onUpdate: (id: string, input: CampaignDraftInput) => void; onSelect: (id: string) => void }) {
  ...
  const [category, setCategory] = useState(2); const [instruction, setInstruction] = useState(2); const [editingId, setEditingId] = useState('');
  ...
  const input = (): CampaignDraftInput => ({ title, summary, severity, category, instruction, profile: 'audible-normal', dataType, ...(dataType === 'regional-record' ? { objectId } : {}), ...(carriesPoint && point ? { latE7: Math.round(point.lat * 1e7), lonE7: Math.round(point.lon * 1e7), radiusM } : dataType === 'official-alert' ? { clearLocation: true } : {}) });
  const submit = (e: FormEvent) => { e.preventDefault(); if (overTextBudget) return; editingId ? onUpdate(editingId, input()) : onCreate(input()); };
  const beginEdit = () => { if (!selected) return; setEditingId(selected.campaignId); setTitle(selected.title); setSummary(selected.summary); setSeverity(selected.severity); setCategory(selected.category); setInstruction(selected.instruction); setDataType(selected.dataType ?? 'official-alert'); setObjectId(selected.objectId ?? records[0]?.objectId ?? ''); setPoint(typeof selected.latE7 === 'number' && typeof selected.lonE7 === 'number' ? { lat: selected.latE7 / 1e7, lon: selected.lonE7 / 1e7 } : null); setRadiusM(selected.radiusM ?? 5000); };
  ...
  return <section className="panel composer-panel"><PanelHead title="Campaign register" aside={`${campaigns.length} versions`} /><div className="campaign-list">{campaigns.map((c) => <button key={c.campaignId} onClick={() => onSelect(c.campaignId)}><span><strong>{c.title}</strong><small>{campaignTypeName(c.dataType)} · {c.campaignId}</small></span><State value={c.state} /></button>)}</div><form className="composer" onSubmit={submit}><div className="composer-heading"><h3>{editingId ? `Revise ${editingId}` : 'New broadcast data'}</h3>{selected && !editingId && <button type="button" className="button ghost" onClick={beginEdit}>Revise selected</button>}{editingId && <button type="button" className="button ghost" onClick={() => setEditingId('')}>Cancel revision</button>}</div><fieldset className="data-type-picker"><legend>Packet type</legend><label><input type="radio" name="dataType" checked={dataType === 'official-alert'} onChange={() => setDataType('official-alert')} /><span><b>Public alert</b><small>Instruction, severity and place</small></span></label><label><input type="radio" name="dataType" checked={dataType === 'regional-record'} onChange={() => setDataType('regional-record')} /><span><b>Map record</b><small>Current resource, hazard or route</small></span></label></fieldset>{dataType === 'regional-record' && <label>Regional object<select value={objectId} onChange={(event) => setObjectId(event.target.value)} required>{records.map((record) => <option key={record.objectId} value={record.objectId}>{record.name} · {record.state}</option>)}</select></label>}<label>Operator title<input value={title} onChange={(e) => setTitle(e.target.value)} required aria-describedby="title-budget" /></label><small id="title-budget" className={titleBytes > 72 ? 'field-error' : 'field-hint'}>{titleBytes}/72 UTF-8 bytes</small><label>Public instruction<textarea value={summary} onChange={(e) => setSummary(e.target.value)} required aria-describedby="summary-budget" /></label><small id="summary-budget" className={summaryBytes > 140 ? 'field-error' : 'field-hint'}>{summaryBytes}/140 UTF-8 bytes</small><div className="inline-fields"><label>Severity<select value={severity} onChange={(e) => setSeverity(Number(e.target.value))}><option value="0">Information</option><option value="1">Assistance</option><option value="2">Urgent</option><option value="3">Life critical</option></select></label></div>{dataType === 'official-alert' && <div className="inline-fields"><label>Alert category<select value={category} onChange={(e) => setCategory(Number(e.target.value))}>{['Evacuation', 'Shelter in place', 'Weather', 'Utility', 'Health', 'Security', 'All clear'].map((label, value) => <option value={value} key={label}>{label}</option>)}</select></label><label>Prepared instruction<select value={instruction} onChange={(e) => setInstruction(Number(e.target.value))}>{['No coded instruction', 'Evacuate now', 'Move to higher ground', 'Stay indoors', 'Boil water', 'Avoid area', 'Report to shelter', 'Await instruction'].map((label, value) => <option value={value} key={label}>{label}</option>)}</select></label></div>}{dataType === 'official-alert'
    ...
    : <p className="composer-note">A map record broadcasts the object's own stored coordinates.</p>}<button className="button primary" disabled={overTextBudget}>{editingId ? 'Save as new campaign version' : 'Create packet draft'}</button></form></section>;
}
...
function CampaignApproval({ campaign, onAction, onPrepare }: { campaign: Campaign; onAction: (state: CampaignState) => void; onPrepare: () => void }) {
  const next: CampaignState | undefined = campaign.state === 'draft' ? 'validated' : campaign.state === 'validated' ? 'approved' : campaign.state === 'approved' ? 'broadcaster-ready' : campaign.state === 'decode-tested' ? 'scheduled' : campaign.state === 'played' ? 'archived' : undefined;
  return <div className="approval"><div className="approval-copy"><div className="approval-labels"><State value={campaign.state} /><span>{campaignTypeName(campaign.dataType)}</span></div><h2>{campaign.title}</h2><p>{campaign.summary}</p></div><dl className="facts"><Fact label="Tier 2 bytes" value={String(campaign.preview.totalTier2Bytes)} /><Fact label="Frames" value={String(campaign.preview.items[0]?.frameCount ?? 0)} /><Fact label="Repetitions" value={`${campaign.preview.items[0]?.repeats ?? 0}×`} /><Fact label="Duration / budget" value={`${campaign.preview.totalDurationS}s / ${campaign.preview.budgetS}s`} /><Fact label="Broadcast point" value={coordinateText(campaign.latE7, campaign.lonE7)} /><Fact label="Radius" value={campaign.radiusM ? `${(campaign.radiusM / 1000).toFixed(campaign.radiusM % 1000 === 0 ? 0 : 1)} km` : '—'} /></dl>{campaign.preview.overBudget && <p className="field-error" role="alert">This campaign exceeds the audio-duration budget and cannot be validated.</p>}<details className="inventory"><summary>Inspect packet inventory and repetition schedule</summary>{campaign.preview.items.map((item) => <div key={item.packetId}><code>{item.packetId}</code><span>{item.tier1Bytes} B Tier 1 · {item.tier2Bytes} B Tier 2 · {item.frameCount} frames · {item.repeats} repetitions</span></div>)}<p>{campaign.preview.burstSchedule.length} scheduled frame bursts in the immutable plan.</p></details>{campaign.approvalDigest && <div className="digest"><span>Immutable approval digest</span><code>{campaign.approvalDigest}</code></div>}<div className="action-line">{next && <button className="button primary" onClick={() => onAction(next)}>{next === 'validated' ? 'Validate packet budget' : next === 'approved' ? 'Approve this exact version' : next === 'broadcaster-ready' ? 'Send to broadcast desk' : next === 'scheduled' ? 'Schedule verified program' : 'Archive played campaign'}</button>}{campaign.state === 'broadcaster-ready' && <button className="button primary" onClick={onPrepare}>Generate WavePX program</button>}</div></div>;
}

function BroadcastConsole({ campaign, onRefresh, receiverOnly = false }: { campaign?: Campaign; onRefresh: () => Promise<void>; receiverOnly?: boolean }) {
  const [mode, setMode] = useState<'send' | 'receive'>(() => receiverOnly ? 'receive' : 'send'); const [state, setState] = useState('Ready'); const [frames, setFrames] = useState<string[]>([]); const [frameSource, setFrameSource] = useState<'tier2-mic' | 'tier2-direct'>('tier2-mic'); const [level, setLevel] = useState(0); const [busy, setBusy] = useState(false); const [decoded, setDecoded] = useState(campaign?.decodeResult?.decodedMessage); const link = useRef<Tier2AudioLink>(); const submitted = useRef(''); const previewRun = useRef(0);
  ...
  const exportWav = async () => { if (!campaign.decodeResult?.passed || !['decode-tested', 'scheduled'].includes(campaign.state)) { setState('Pass exact decode testing before export.'); return; } setBusy(true); try { const modem = makeLink(); await modem.init(); const blob = modem.createWav(program.playbackFramesBase64.map(decodeBase64), program.profile); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${program.programId}.wav`; a.click(); URL.revokeObjectURL(url); await api.recordBroadcastEvent(campaign.campaignId, 'exported'); setState('WAV exported and recorded against the tested artifact'); await onRefresh(); } catch (reason) { setState(errorText(reason)); } finally { setBusy(false); } };
  const previewAudio = async () => { const run = ++previewRun.current; setBusy(true); setState('Preparing audio preview'); try { await makeLink().transmit(program.playbackFramesBase64.map(decodeBase64), program.profile, (sent, total) => { if (run === previewRun.current) setState(`Playing preview · frame ${sent} of ${total}`); }); if (run === previewRun.current) setState('Audio preview complete'); } catch (reason) { if (run === previewRun.current) setState(errorText(reason)); } finally { if (run === previewRun.current) setBusy(false); } };
  const stopAudio = () => { previewRun.current += 1; link.current?.stopTransmission(); setBusy(false); setState('Audio preview stopped'); };
  ...
  return <div className="broadcast-console">{!receiverOnly && <div className="mode-tabs"><button className={mode === 'send' && !receiverOnly ? 'active' : ''} onClick={() => setMode('send')}>Transmit</button><button className={mode === 'receive' ? 'active' : ''} onClick={() => setMode('receive')}>Decode audio</button></div>}<div className="audio-preview"><div><strong>GGWave audio preview</strong><span>{program.playbackFramesBase64.length} frames · {program.profile} · plays through this device</span></div><div className="action-line"><button className="button primary" disabled={busy} onClick={() => void previewAudio()}>Play audio preview</button><button className="button ghost" onClick={stopAudio}>Stop audio</button></div></div><div className="station-state" role="status"><span className="live-dot" /><strong>{state}</strong></div>{mode === 'send' && !receiverOnly ? <><div className="program-strip"><span><small>Program</small><code>{program.programId}</code></span><span><small>Verification</small><strong>{campaign.decodeResult?.passed ? 'Matched approved packet' : 'Decode required'}</strong></span><span><small>Sequence</small><strong>{program.playbackFramesBase64.length} frames</strong></span></div><div className="action-line"><button className="button primary" disabled={busy || !canPlay} onClick={() => void transmit()}>Play scheduled program</button><button className="button secondary" disabled={busy || !canExport} onClick={() => void exportWav()}>Export WAV</button></div>{!canExport && <p className="field-hint">Decode the complete program before export or playback.</p>}</> : <><div className="receiver-meter" aria-label="Microphone input level"><i style={{ width: `${Math.min(100, level * 900)}%` }} /></div><strong className="frame-count">{frames.length}/{expected} frames · {frameSource === 'tier2-mic' ? 'microphone' : 'WAV file'}</strong><div className="action-line"><button className="button primary" onClick={() => void listen()}>Listen to microphone</button><button className="button ghost" onClick={() => link.current?.stopListening()}>Stop</button><label className="button secondary file-button">Choose WAV<input type="file" accept="audio/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) void decodeFile(file); }} /></label><button className="button secondary" disabled={busy || frames.length === 0} onClick={() => void submitReception(frameSource)}>Compare frames</button></div>{decoded && <div className="decoded-message"><span>Recovered message</span><h3>{alertCategoryName(decoded.category)} · Severity {decoded.severity}</h3><p>{decoded.text}</p><dl><Fact label="Campaign" value={decoded.campaignId} /><Fact label="Region" value={decoded.regionCode} /><Fact label="Language" value={decoded.language} /><Fact label="Recovered point" value={coordinateText(decoded.location?.latE7, decoded.location?.lonE7)} /><Fact label="Radius" value={decoded.location?.radiusM ? `${(decoded.location.radiusM / 1000).toFixed(decoded.location.radiusM % 1000 === 0 ? 0 : 1)} km` : '—'} /></dl>{decoded.location && <p className={decoded.location.matchesApproved ? 'verify-chip pass' : 'verify-chip fail'}>{decoded.location.matchesApproved ? 'Coordinates match the approved packet' : 'Coordinates differ from the approved packet'}</p>}</div>}</>}<details className="audio-evidence"><summary>Program frames and event history</summary><dl><Fact label="Artifact digest" value={program.artifactDigest} /><Fact label="Unique frames" value={String(program.uniqueFramesBase64.length)} /><Fact label="Playback frames" value={String(program.playbackFramesBase64.length)} /><Fact label="Recovered now" value={String(frames.length)} /></dl><div className="frame-ledger">{program.uniqueFramesBase64.map((frame, index) => <div key={`${frame.slice(0, 16)}-${index}`}><span>Expected {index + 1}</span><code>{frame}</code><b>{Math.floor(frame.length * .75)} B encoded</b></div>)}{frames.map((frame, index) => <div className="recovered" key={`recovered-${frame.slice(0, 16)}-${index}`}><span>Recovered {index + 1}</span><code>{frame}</code><b>{program.uniqueFramesBase64.includes(frame) ? 'Expected match' : 'Unexpected'}</b></div>)}</div>{campaign.decodeResult && <div className="decode-proof"><span>{campaign.decodeResult.receptionTransport === 'tier2-mic' ? 'Microphone recovery' : 'WAV recovery'}</span><strong>{campaign.decodeResult.passed ? 'Exact comparison passed' : 'Comparison failed'}</strong><small>{campaign.decodeResult.recoveredFrames}/{campaign.decodeResult.expectedFrames} recovered · {campaign.decodeResult.corruptFrames} corrupt · {campaign.decodeResult.unexpectedFrames} unexpected · {campaign.decodeResult.missingFrames} missing</small><small>{campaign.decodeResult.canonicalMatch ? 'Recovered bytes match the approved packet' : 'Recovered bytes do not match the approved packet'}</small>{campaign.decodeResult.reassembledDigest && <small>SHA-256 {campaign.decodeResult.reassembledDigest.slice(0, 24)}…</small>}</div>}<div className="broadcast-events">{(campaign.broadcastEvents ?? []).map((event) => <div key={event.eventId}><strong>{event.event}</strong><span>{formatDateTime(event.atMs)} · {event.operatorLabel}</span><code>{event.programId} · {event.artifactDigest.slice(0, 20)}…</code></div>)}</div></details></div>;
}
...
function gatewayLabel(nodeToken: string): string { return nodeToken.replace(/^assam-?/i, '').replaceAll('-', ' ').replace(/\b\w/g, (value) => value.toUpperCase()); }
...
function gatewayActivity(audit: GatewayAudit, token: string): { ageMs: number; uploads: number; downloads: number } {
  ...
  const lastAtMs = Math.max(0, ...transfers.map((item) => item.atMs), ...observationTimes);
  return {
