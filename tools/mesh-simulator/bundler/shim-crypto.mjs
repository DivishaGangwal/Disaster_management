/**
 * Browser stand-in for `node:crypto`, used only by @dsm/codec's integrity module.
 *
 * It needs exactly two things, and both must be SYNCHRONOUS because
 * payloadDigest() is synchronous on the hot path:
 *
 *   createHash('sha256').update(bytes).digest('hex')
 *   randomBytes(n)
 *
 * Web Crypto's digest is async, so SHA-256 is implemented here directly (FIPS
 * 180-4). randomBytes uses the platform CSPRNG.
 *
 * This changes NO protocol behaviour: the same bytes in produce the same digest
 * out, which the build script verifies against Node's own crypto before the
 * bundle is written.
 */
